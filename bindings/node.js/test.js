...
let cur_prefix = Buffer.from("D".concat(key_b64)) // attach derivation code.
let next_prefix = Buffer.from("D".concat(next_key_b64))
let new_next_prefix = Buffer.from("D".concat(new_next_key_b64))
...
let sign_prefix = Buffer.from("0B".concat(signature_b64)) // attach derivation code.
...
sign_prefix = Buffer.from("0B".concat(signature_b64)) // attach derivation code.
...
signature_b64 = Base64.encode(signature, true);
sign_prefix = Buffer.from("0B".concat(signature_b64))
