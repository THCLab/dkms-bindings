...
pub mod kel;
use kel::{
    event_generator::PublicKeysConfig,
    KEL,
};
...
fn rotate(ctx: CallContext) -> JsResult<JsBuffer> {
    ...
    let rot_event = kel.rotate(&pub_keys).expect("No rotation event").serialize().unwrap();
...
fn get_current_public_key(ctx: CallContext) -> JsResult<JsBuffer> {
    ...
    let key = kel.get_current_public_keys(&prefix).expect("Error while processing").unwrap()[0].key.clone();
...
fn verify(ctx: CallContext) -> JsResult<JsBoolean> {
    ...
    let this: JsObject = ctx.this_unchecked();
    let kel: &KEL = ctx.env.unwrap(&this)?;
    ctx.env.get_boolean(kel.verify(&message, &signature, &identifier).expect("Error while verifing"))
}
...
fn verify_at_sn(ctx: CallContext) -> JsResult<JsBoolean> {
    ...
    let this: JsObject = ctx.this_unchecked();
    let kel: &KEL= ctx.env.unwrap(&this)?;
    ctx.env.get_boolean(kel.verify_at_sn(&message, &signature, &identifier, sn as u64).expect("Error while verifing"))
}
