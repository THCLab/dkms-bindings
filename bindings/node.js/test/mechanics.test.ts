...
async function publish(identifier, sigType, currentKeyManager) {
  await identifier.notifyWitness();
    ...
    let qrySignaturePrefix = new mechanics.Signature(sigType, Buffer.from(qry_signature));
...
describe("Mechanics", () => {
  it("Issue VC", async () => {
    ...
    let signaturePrefix = new mechanics.Signature(sigType, Buffer.from(signature));
    ...
    let ixnSignaturePrefix = new mechanics.Signature(sigType, Buffer.from(ixnSignature));
    ...
    let issueIxnSignaturePrefix = new mechanics.Signature(sigType, Buffer.from(issueIxnSignature));
    ...
    let verifierConfig = new mechanics.ConfigBuilder().withDbPath(verifierTmpFileName).build();
    let verifier = new mechanics.Controller(verifierConfig);

    let verifierPk = new mechanics.PublicKey(keyType, Buffer.from(currentVerifierKeyManager.pubKey));
    let verifierPk2 = new mechanics.PublicKey(keyType, Buffer.from(nextVerifierKeyManager.pubKey));
    ...
    let verifierSignaturePrefix = new mechanics.Signature(sigType, Buffer.from(verifierSignature));
    ...
    let addWatcherSignaturePrefix = new mechanics.Signature(sigType, Buffer.from(addWatcherSignature));
    ...
    for (let item of kelQueries) {
      ...
      let kelQrySigPrefix= new mechanics.Signature(sigType, Buffer.from(kelQrySignature));
    ...
    for (var element of [1,2,3, 4, 5]) {
      ...
      let telQrySigPrefix= new mechanics.Signature(sigType, Buffer.from(telQrySignature));
