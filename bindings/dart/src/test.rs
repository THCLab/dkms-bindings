...
use crate::api::{add_watcher, finalize_event, propagate_oobi, query, resolve_oobi};
...
pub fn test_process() -> Result<()> {
    ...
    let kel = get_kel("EZrJQSdhdiyXNpEzHo-dR0EEbLfcIopBSImdLnQGOKkg")?;
    println!("kel: {}", kel);
...
pub fn test_parse_attachment() -> Result<()> {
    ...
    process_stream(test_kel.to_string())?;
    let _kel = get_kel("Ew-o5dU5WjDrxDBK4b4HrF82_rYb6MX6xsegjq4n0Y7M")?;
...
pub fn test_add_watcher() -> Result<()> {
    ...
    let watcher_oobi = r#"{"eid":"BmiAovtQANjpKjXCymso4uO-lt7zUFVzU2WRWL4WJOqg","scheme":"http","url":"http://127.0.0.1:3236/"}"#;
    println!("\nResolving watcher oobi: \n{}", watcher_oobi);
    resolve_oobi(watcher_oobi.into())?;

    let add_watcher_message = add_watcher(
        &controller,
        &"BmiAovtQANjpKjXCymso4uO-lt7zUFVzU2WRWL4WJOqg"
            .parse()
            .unwrap(),
    )?;
    println!(
        "\nController generate end role message to add witness: \n{}",
        add_watcher_message
    );
    ...
    finalize_event(&controller, add_watcher_message, signature).unwrap();

    let issuer_oobi = r#"{"cid":"E6PArG3bN0XpIzwC9Ar9Smu1KPJcICUDJadtccNk4BiY","role":"witness","eid":"BMOaOdnrbEP-MSQE_CaL7BhGXvqvIdoHEMYcOnUAWjOE"}"#;
    println!("\nSending issuer oobi to watcher: \n{}", issuer_oobi);
    propagate_oobi(&controller, issuer_oobi.into()).unwrap();

    println!("\nQuering about issuer kel...");
    let iss_id = "E6PArG3bN0XpIzwC9Ar9Smu1KPJcICUDJadtccNk4BiY";
    query(&controller, iss_id).unwrap();

    let issuer_kel = get_kel(iss_id)?;
    println!("\nIssuer kel: \n{}", issuer_kel);
