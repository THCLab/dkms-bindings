...
use crate::api::{add_watcher, resolve_oobi};
...
pub fn test_add_watcher() -> Result<()> {
    ...
    let oobi = r#"{"eid":"BCnScHVViRjj7vwPrbr_4Jehye1uypY9dlSDDr4KLDww","scheme":"http","url":"http://127.0.0.1:3236/"}"#;

    resolve_oobi(oobi.into())?;
    let r = add_watcher(
        controller,
        &"BCnScHVViRjj7vwPrbr_4Jehye1uypY9dlSDDr4KLDww"
            .parse()
            .unwrap(),
    )?;
    println!("watcher add = {}", r);
